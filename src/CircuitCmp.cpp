...
bool CircuitCmp::CheckInputEqual2(Gate* x, Gate* y){
	if(x->total_input_name.size()!= y->total_input_name.size() ||x->total_cut_name.size()!=y->total_cut_name.size() )
    return false;
 	for(int i=0;i<x->total_input_name.size();++i){
		if(x->total_input_name[i]!=y->total_input_name[i])
			return false;
	}
	
	for(int i=0;i<x->total_cut_name.size();++i)
		if(x->total_cut_name[i]!=y->total_cut_name[i])
			return false;
	return true;
}
...
void CircuitCmp::Replace(Gate* gate){
	...
	gate->total_input_name=gate->input[0]->total_input_name;	
	for(int i=1;i<gate->input.size();++i){
		full=false;		
		//cout<<"A"<<endl;
		for(int s=0,j=0;s<gate->total_input_name.size() || j<gate->input[i]->total_input_name.size();){			
			if(!full){
				if(s==gate->total_input_name.size() || j==gate->input[i]->total_input_name.size())
					full=true;
			}			
...
void CircuitCmp::CheckDivision(){
		...
		if(circuitOne->GetoutputSize()==circuitTwo->GetoutputSize()){
			...
			for(int i=0;i<outputsize;i++){
				if(circuitOne->Getoutput(i)->total_input_name!=circuitTwo->Getoutput(i)->total_input_name){
